            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                if len(ep_cost) == 0:
                    ...
                    if self.penalty_param<0:
                        self.penalty_param = torch.tensor(0.0, requires_grad=True)
                    # self.penalty_param = torch.clamp(self.penalty_param, min=0.0)
                    # print("penalty_param",self.penalty_param)
                    self.penalty_item2 = softplus(self.penalty_param).item()
                    wandb.log({"reward": reward_sum, "costs": cost_sum, "penalty_param": self.penalty_item2})
