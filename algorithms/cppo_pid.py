...
class CPPO_PID:
    ...
    def __init__(self,
        ...
        self.pid_Kd = 0.01
        self.pid_d_delay = 10
        self.pid_delta_p_ema_alpha = 0.95
        ...
        self.pid_i = self.cost_penalty = 0.001
        ...
        wandb.init(project="temp_for_safehand", name=self.log_dir.split('/')[-3] + " " + self.log_dir.split('/')[-2] + " " + currtime, entity="pku_rl")
        config = wandb.config
        config.learning_rate = 0.01

    def pid_update(self, ep_cost_avg):
        delta = float(ep_cost_avg - self.cost_lim)
        ...
        if self.is_testing:
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                if len(ep_cost) == 0:
                    ...
                    wandb.log({"reward": statistics.mean(rewbuffer), "costs": statistics.mean(costbuffer), "penalty_param": self.penalty_param})
    ...
    def update(self):
        ...
        for epoch in range(self.num_learning_epochs):
            ...
            for indices in batch:
                ...
                self.penalty = self.cost_penalty
                # print(self.penalty)
                # print(penalty)
                # exit(0)
                loss = -surrogate_loss + self.penalty * surrogate_cost_loss - self.entropy_coef * entropy_batch.mean()
