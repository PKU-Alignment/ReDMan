...
import copy
class ShadowHandAllegroHandJenga(BaseTask):
...
# region: init position
        ...
        shadow_hand_start_pose.p = gymapi.Vec3(0, -1.00, 0.06)
        shadow_hand_start_pose.r = gymapi.Quat(-1,0,0,1)*gymapi.Quat(math.sin(math.radians(-20)),0,0,math.cos(math.radians(-20)))

        allegro_hand_start_pose = gymapi.Transform()
        allegro_hand_start_pose.p = gymapi.Vec3(-0.01, -0.32, 0.44)
        allegro_hand_start_pose.r = gymapi.Quat(-1,0,0,1)*gymapi.Quat(0,1,0,0)*gymapi.Quat(0,0,math.sin(math.radians(45)),math.cos(math.radians(45)))*gymapi.Quat(0,math.sin(math.radians(-10)),0,math.cos(math.radians(-10)))
...
# endregion
        ...
        for i in range(self.num_envs):
            ...
            # print(self.gym.find_actor_rigid_body_index(env_ptr,shadow_hand_actor,'robot0:forearm',gymapi.DOMAIN_SIM))
            # print(self.gym.find_actor_rigid_body_index(env_ptr,allegro_hand_actor,'th4',gymapi.DOMAIN_SIM))
            # exit()
            ...
            for object_shape_prop in object_shape_props:
                object_shape_prop.friction = 0.01
        ...
        self.fingertip_another_handles_env = to_torch(self.fingertip_another_handles_env, dtype=torch.long, device=self.device) + 26
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        fingertip_another_obs_start = another_hand_start + 48 # 16*3
        ...
        self.obs_buf[:, action_another_obs_start:action_another_obs_start + 22] = self.actions[:, 26:] # 16+6
...
def compute_hand_reward(
    ...
    dist_reward = - right_hand_finger_dist
