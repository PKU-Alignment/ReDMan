...
class ShadowHandGrasp(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        object_asset_options.density = 5
        ...
        shadow_hand_start_pose.p = gymapi.Vec3(*get_axis_params(0.1, self.up_axis_idx))
        shadow_hand_start_pose.r.x, shadow_hand_start_pose.r.y, shadow_hand_start_pose.r.z, shadow_hand_start_pose.r.w = [0, 1, 0, 0]

        object_start_pose = gymapi.Transform()
        object_start_pose.p = gymapi.Vec3()
        object_start_pose.p.x = shadow_hand_start_pose.p.x
        pose_dz, pose_dy = -0.1, -0.4

        object_start_pose.p.y = shadow_hand_start_pose.p.y + pose_dy
        object_start_pose.p.z = shadow_hand_start_pose.p.z + pose_dz
        # print(object_start_pose.p.z)

        if self.object_type == "pen":
            object_start_pose.p.z = shadow_hand_start_pose.p.z + 0.02
        goal_delta_z = 10.1
        self.goal_displacement = gymapi.Vec3(0.0, 0.0, 0.12+goal_delta_z)
        ...
        for i in range(self.num_envs):
            ...
            for object5_shape_prop in object_shape_props:
                object5_shape_prop.friction = 0.5
    ...
    def compute_cost(self, actions):
        ...
        self.cost_buf = torch.where(actions[:, 2] < -0.9, torch.ones_like(self.cost_buf), self.cost_buf)
        self.cost_buf = torch.where(actions[:, 2] > 0.9, torch.ones_like(self.cost_buf), self.cost_buf)
        self.cost_buf = torch.where(actions[:, 3] < -0.9, torch.ones_like(self.cost_buf), self.cost_buf)
        self.cost_buf = torch.where(actions[:, 3] > 0.9, torch.ones_like(self.cost_buf), self.cost_buf)
        self.cost_buf = torch.where(actions[:, 4] < -0.9, torch.ones_like(self.cost_buf), self.cost_buf)
        self.cost_buf = torch.where(actions[:, 4] > 0.9, torch.ones_like(self.cost_buf), self.cost_buf)
...
def compute_hand_reward(
    ...
    reward = h_rew*100#dist_rew + rot_rew + action_penalty * action_penalty_scale
    print(reward[0])

    # Find out which envs hit the goal and update successes count
    goal_resets = torch.where(h_rew >= 0.1, torch.ones_like(reset_goal_buf), reset_goal_buf)
