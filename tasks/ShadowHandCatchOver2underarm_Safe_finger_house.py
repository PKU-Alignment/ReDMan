...
class ShadowHandCatchOver2Underarm_Safe_finger_house(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        ground_asset = self.gym.create_box(self.sim, 2.0, 3.0, 0.02001, house_asset_options)
        wall1_asset = self.gym.create_box(self.sim, 2.0, 0.02, 2, house_asset_options)
        wall2_asset = self.gym.create_box(self.sim, 0.02, 3.0, 2, house_asset_options)
        ...
        ground_start_pose.p = gymapi.Vec3(0, 0, -0.01)
        ...
        shadow_another_hand_start_pose.r = (gymapi.Quat().from_euler_zyx(0, 3.1415, 3.1415))*gymapi.Quat(0, 1, 0, 1)*gymapi.Quat(0, 1, 0, 1)
...
def compute_hand_reward(
    ...
    right_hand_base_dist = torch.norm(right_hand_base_pos - torch.tensor([0.0, 0.0, 0.5], dtype=torch.float, device=rew_buf.device), p=2, dim=-1)
    left_hand_base_dist = torch.norm(left_hand_base_pos - torch.tensor([0.0, -0.8, 0.5], dtype=torch.float, device=rew_buf.device), p=2, dim=-1)
